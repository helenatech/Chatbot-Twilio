    ...
    public class BolsaGraduacaoAdm : IInteractionService
    {
        public async Task<string> HandleAsync(Client client, UserState userState)
        {
            switch (userState.CurrentOption)
            {
                case "1":
                    return @"                
                    Benefício destinado a custear a primeira graduação no CEUB.
                    O custeio se refere especificamente ao valor da matrícula e das mensalidades do curso, não incluindo as disciplinas reprovadas e extracurriculares.</li>
